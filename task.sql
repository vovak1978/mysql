...
select max(sum) from application where Currency = 'euro';
select min(sum) from application where Currency = 'gryvnia';

#13. Порахувати кількість кредитів для клієнтів,які мають вищу освіту.
select count(idapplication) from application a join client c on Client_idClient = idClient
...
select idDepartment, sum(sum) as suma from 
...
group by idDepartment having s > 10000; 
...
select sum(Sum) as suma
from application;


#26. Знайти клієнтів, які є з того самого міста, що і клієнт, який взяв найбільшу кількість кредитів*/
select FirstName, City,  count(idApplication) as sum
from application as a, client as c 
where a.Client_idClient = c.idClient
group by Client_idClient order by sum desc;
